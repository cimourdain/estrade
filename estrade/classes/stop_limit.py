...
class StopLimitAbsolute(AStopLimit):
    ...
    def _check_value_consistency(self, value):
        ...
        if self.trade.direction > 0 and (
                (self.type_ == 'STOP' and value >= self.trade.open) or
                (self.type_ == 'LIMIT' and value <= self.trade.open)
        ):
            raise StopLimitException(
                'Impossible to set a {} ({}) {} than open tick on a {} opened @ {}'.format(
                    self.type_.upper(),
                    value,
                    'higher' if self.type_ == 'STOP' else 'lower',
                    'BUY' if self.trade.direction > 0 else 'SELL',
                    self.trade.open,
                )
            )
            ...
            raise StopLimitException(
                'Impossible to set a {} ({}) {} than open tick on a {} opened @{}'.format(
                    self.type_.upper(),
                    value,
                    'higher' if self.type_ == 'LIMIT' else 'lower',
                    'BUY' if self.trade.direction > 0 else 'SELL',
                    self.trade.open,
                )
            )
...
class StopLimitRelative(AStopLimit):
    ...
    def direction(self):
        ...
        direction define if stop/limit is above (direction=1) or under (direction=-1) the trade open.
    ...
    def absolute_value(self):
        """
        In a relative stop/limit, calculate the absolute value with trade open and direction
        :return: <int> or <float>
        """
        if self._value:
