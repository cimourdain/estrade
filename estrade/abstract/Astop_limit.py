...
class AStopLimit:
    ...
    def type_(self, type_):
        ...
        if not isinstance(type_, str) or type_.upper() not in ['STOP', 'LIMIT']:
            raise StopLimitException('Invalid type {}'.format(type_))
    ...
    def value(self):
        ...
        :return: <int>
        """
        raise StopLimitException('Cannot call value for stop/limit, call absolute_value or relative_value explicitly.')
    ...
    def _check_value_consistency(self, value):
        """
        This method must be implemented to check that the provided stop/limit value is consistent with trade open.
    ...
    def check(self, tick):
        ...
        if self.type_ == 'STOP':
            if self.trade.direction > 0 and tick.bid <= self.absolute_value:
                logger.debug('close BUY because tick %f  <= Stop absolute value %f' % (
                    tick.bid,
                    self.absolute_value
                ))
                return True
            elif self.trade.direction < 0 and tick.ask >= self.absolute_value:
                logger.debug('close SELL because tick %f >= Stop absolute value %f' % (
                        tick.ask,
                        self.absolute_value
                ))
                return True
            ...
            if self.trade.direction > 0 and tick.bid >= self.absolute_value:
                logger.debug('close BUY because tick %f >= limit absolute value %f' % (
                        tick.bid,
                        self.absolute_value
                ))
                return True
            elif self.trade.direction < 0 and tick.ask <= self.absolute_value:
                logger.debug('close SELL because tick %f <= limit absolute value %f' % (
                        tick.ask,
                        self.absolute_value
                ))
                return True
